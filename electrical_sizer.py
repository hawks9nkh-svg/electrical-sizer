...
emergency_pct = st.sidebar.number_input("Emergency Load % of Total", 0, 100, 20)
...
emergency_demand_kva = total_demand_kva * (emergency_pct / 100)
...
with col1:
    ...
    st.metric("**Total Normal Demand kVA**", f"{total_demand_kva:.1f}")
    st.metric("Emergency Demand kVA", f"{emergency_demand_kva:.1f} kVA")
with col2:
    grand_total = total_demand_kva + emergency_demand_kva
    service_amps = (grand_total * 1000 / (480 * 1.732)) * 1.25 * (1 + spare_pct/100)
    recommended_service = max(100, round(service_amps / 50) * 50)
    st.metric("Recommended Service (480V example)", f"{recommended_service} A")
    st.metric("Emergency Generator (rough)", f"{int(emergency_demand_kva * 1.3):.0f} kW")

st.info("All demand factors and emergency % editable on left. Restaurant kitchen included.")

if st.button("Show Load Breakdown"):
    st.dataframe(pd.DataFrame({
        "Component": ["General", "Kitchen", "HVAC", "Normal Total", "Emergency", "Grand Total"],
        "kVA": [demand_general_kva, kitchen_demand_kva, hvac_demand_kva, total_demand_kva, emergency_demand_kva, grand_total]
    }))
