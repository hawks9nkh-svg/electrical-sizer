...
with tab1:
    ...
    with right:
        ...
        service_amps = (total_normal_kva * 1000 / (480 * 1.732)) * 1.25 * (1 + spare_pct/100)
        ...
        st.metric("Emergency Generator (rough - normal only)", f"{int(total_normal_kva * 0.2 * 1.3):.0f} kW")
...
st.info("Separate tabs as requested. All inputs on left in Normal tab, results on right. Fire Alarm and Fire Pump added to Emergency.")

if st.button("Show Full Breakdown"):
